...
import { useSkillsSection } from "../context/ResumeContext";
...
export default function Skills() {
  ...
  const [items = [], setItems] = useSkillsSection();
  ...
  function remove(i: number) {
    ...
    next.splice(i, 1);
    setItems(
      next.length ? next : [{ name: "", category: "Programming", level: 3 }]
    );
  }
  ...
  function upsertSkill(
    ...
    if (idx >= 0)
      update(idx, { level: Math.max(items[idx].level, level), category });
      ...
      <section className="mx-auto max-w-6xl px-4 py-6 grid lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2">
          ...
          <p className="text-gray-600 mt-1">
            Show your strongest technologies and tools. Recruiters scan this
            section first.
          </p>
              ...
              {items.map((s, i) => (
                <div key={i} className="rounded-2xl  bg-white p-3">
                  <div className="grid md:grid-cols-[1fr,160px,160px,auto] gap-3 items-center">
                    <span className="rounded-lg  px-3 py-2 text-sm focus:ring-2 focus:ring-sky-200">
                      {s.name}
                    </span>
                    <CategorySelect
                      value={s.category}
...
function guessCategory(name: string): string {
  ...
  if (/communication|leadership|ownership|collaboration|mentoring/.test(n))
    return "Soft Skills";
  return "Programming";
}
...
function CategorySelect({
  ...
  return (
    <select
      className="rounded-lg border px-3 py-2 text-sm focus:ring-2 focus:ring-sky-200"
      value={value}
  ...
  return (
    <div className="grid grid-cols-[1fr,44px] items-center gap-2">
      <input
        ...
        className="w-full"
      />
      <div className="text-xs text-gray-700 rounded-md border px-2 py-1 text-center">
        {value}/5
      </div>
    </div>
  );
    ...
    <div className="mt-5 rounded-2xl border bg-white p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-semibold">
          <Sparkles className="h-4 w-4 text-sky-600" /> Quick add from
          suggestions
        </div>
        <div className="relative">
          <Search className="absolute left-2 top-2.5 h-4 w-4 text-gray-400" />
          <input
            className="w-64 rounded-lg border pl-7 pr-3 py-2 text-sm focus:ring-2 focus:ring-sky-200"
      ...
      <div className="mt-4 grid md:grid-cols-[1fr,auto] gap-3">
        <input
          ...
          placeholder="Or paste comma‑separated skills: React, Node.js, MongoDB"
        />
        <button
          onClick={importCsv}
...
function SkillPreview({
  ...
  items: { name: string; category: string; level: number }[];
}) {
          ...
          <div className="space-y-2">
            {grouped[c]
              ...
              .map((s, i) => (
                <div
                  key={i}
                  className="grid grid-cols-[1fr,90px] gap-3 items-center text-sm"
                >
            ...
            {i < current ? (
              ...
              <span
                className={`h-2 w-2 rounded-full ${
                  i === current ? "bg-sky-600" : "bg-gray-300"
                }`}
              ></span>
            )}
