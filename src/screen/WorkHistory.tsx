...
import { useWorkHistorySection } from "../context/ResumeContext";
...
export default function WorkHistory() {
  ...
  const [items = [], setItems] = useWorkHistorySection();
  ...
  function remove(i: number) {
    const next = [...items];
    next.splice(i, 1);
    setItems(next.length ? next : [{ role: "", company: "", bullets: [] } as Entry]);
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="mx-auto max-w-6xl px-4 py-6 grid lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2">
          ...
          <p className="text-gray-600 mt-1">Add your roles, responsibilities and key achievements.</p>

          <div className="mt-6 space-y-5">
            {items.map((e, i) => (
              <div key={i} className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <div className="text-sm font-semibold">Position #{i+1} {isValid(e) && <CheckCircle2 className="inline h-4 w-4 text-emerald-600 ml-1"/>}</div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <Field label="Job Title *" value={e.role} onChange={(v)=>update(i,{ role: v })} placeholder="e.g., Senior Software Engineer" />
                  <Field label="Company *" value={e.company} onChange={(v)=>update(i,{ company: v })} placeholder="e.g., Acme Inc" />
                </div>
                ...
                <div className="mt-3">
                  ...
                  <Bullets value={e.bullets} onChange={(v)=>update(i,{ bullets: v })} />
                </div>
              </div>
            ))}
      ...
      {/* <div className="border-t bg-white sticky bottom-0">
        <div className="mx-auto max-w-6xl px-4 py-4 flex flex-col sm:flex-row gap-3 items-center justify-between">
          <button onClick={()=>nav('/builder/heading')} className="inline-flex items-center gap-2 rounded-2xl border px-4 py-2 text-sm">
            <ArrowLeft className="h-4 w-4"/> Back: Heading
          </button>
      ...
      <Footer nextSectionName="Education" prevSectionName="Heading" prevSection={() => nav("/builder/heading")} nextSection={saveAndNext} saving={false} />
    </div>
  );
}
...
function Bullets({ value, onChange }: { value: string[]; onChange: (v:string[])=>void }) {
  ...
  return (
    <div className="mt-1">
      <textarea className="w-full min-h-[120px] rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-200" placeholder="Use one line per bullet (e.g., Reduced API latency by 40%)" value={text} onChange={(e)=>{ setText(e.target.value); onChange(normalize(e.target.value)); }} />
      <div className="mt-1 text-xs text-gray-500">Tip: Start bullets with an action verb and a metric (e.g., "Led", "Improved", "Reduced by 20%").</div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border bg-white p-3">
      <div className="flex items-start justify-between">
        <div>
          <div className="font-semibold leading-tight">{e.role || 'Job Title'}</div>
          <div className="text-xs text-gray-600">{e.company || 'Company'}{e.city ? ` • ${e.city}` : ''}{e.country ? `, ${e.country}` : ''}</div>
        </div>
      ...
      {e.bullets?.length ? (
        ...
        <div className="mt-2 text-xs text-gray-500">Add 2–5 bullets that quantify impact.</div>
      )}
    </div>
  );
}
